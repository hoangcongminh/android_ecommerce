...
import com.example.markeetapp.adapter.AdapterProductImage;
...
import com.example.markeetapp.model.ProductImage;
import com.example.markeetapp.model.Wishlist;
...
public class ActivityProductDetails extends AppCompatActivity {
    ...
    private void toggleCartButton() {
        if (flag_cart) {
            ...
            // check stock product
            if (product.stock == 0 || product.status.equalsIgnoreCase("HẾT HÀNG")) {
                Toast.makeText(this, R.string.msg_out_of_stock, Toast.LENGTH_SHORT).show();
                return;
            }
            if (product.status.equalsIgnoreCase("TẠM NGƯNG")) {

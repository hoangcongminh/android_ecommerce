...
class DASHBOARD extends REST{
    ...
    public function findDashboardProductData(){
        ...
        $order['waiting'] = $this->product_order->countByStatusPlain('ĐANG CHỜ');
        $order['processed'] = $this->product_order->countByStatusPlain('ĐÃ XỬ LÍ');
        ...
        $product['ready_stock'] = $this->product->countByStatusPlain('CÓ HÀNG');
        $product['out_of_stock'] = $this->product->countByStatusPlain('HẾT HÀNG');
        $product['suspend'] = $this->product->countByStatusPlain('TẠM NGƯNG');

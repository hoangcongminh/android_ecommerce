...
class NewsInfo extends REST{
	...
	public function insertOne(){
		...
		if($data['status'] == 'NỔI BẬT' && $data['draft'] == 0 && $this->isFeaturedExceed() == 1){
	...
	public function updateOne(){
		...
		$table_name = 'news_info';
		$pk = 'id';
		if($data[$table_name]['status'] == 'NỔI BẬT' && $data[$table_name]['draft'] == 0 && $this->isFeaturedExceed() == 1){
			$msg = array('status' => "failed", "msg" => "Featured News exceed the maximum amount", "data" => null);
			$this->show_response($msg);
		}
        if($data[$table_name]['status'] == 'THÔNG THƯỜNG' && $this->countFeaturedPlain() <= 1){
	...
	public function deleteOne(){
        ...
        if($data['status'] == 'NỔI BẬT' && $this->countFeaturedPlain() <= 1){
            $m = array('status' => "failed", "msg" => "Ops, At least there is one FEATURED news", "data" => null);
            $this->show_response($m);
            return;
        }
	...
	public function findAllFeatured(){
		if($this->get_request_method() != "GET") $this->response('',406);
		$query="SELECT * FROM news_info ni WHERE ni.status='NỔI BẬT' AND ni.draft=0 ORDER BY ni.id DESC";
		return $this->db->get_list($query);
	}
	...
	public function countFeaturedPlain(){
		$query = "SELECT COUNT(DISTINCT ni.id) FROM news_info ni WHERE ni.status='NỔI BẬT' AND ni.draft=0 ";
		return $this->db->get_count($query);
	}
	
	public function isFeaturedExceed(){
		$key_code = 'FEATURED_NEWS';
		$query=	"SELECT COUNT(counter) AS resp from (SELECT COUNT(id) AS counter FROM news_info WHERE status = 'NỔI BẬT' AND draft = 0) as N "

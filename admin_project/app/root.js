angular.module('App').controller('RootCtrl', function ($rootScope, $scope, $mdSidenav, $mdToast, $mdDialog, $cookies, request, focus) {
    ...
    root.PRODUCT_UPDATE = "Cập nhật sản phẩm";
    root.PRODUCT_NEW = "Sản phẩm mới";
    ...
    self.sidenav = {
        actions: [
            {name: 'TRANG CHỦ', icon: 'store', link: '#dashboard', sub: false},
            {name: 'DANH SÁCH ĐƠN HÀNG', icon: 'event_note', link: '#order', sub: false},
            {name: 'SẢN PHẨM', icon: 'widgets', link: '#product', sub: false},
            {name: 'DANH MỤC', icon: 'dns', link: '#category', sub: false},
            {name: 'TIN TỨC', icon: 'subject', link: '#news', sub: false},
            {name: 'ỨNG DỤNG', icon: 'adb', link: '#application', sub: false},
            {name: 'THÔNG BÁO', icon: 'notifications', link: '#notification', sub: false},
            {name: 'CÀI ĐẶT', icon: 'settings', link: '#setting', sub: false},
            {name: 'THÔNG TIN', icon: 'error', link: '#about', sub: false}
        ]
    };
    ...
    self.doLogout = function (ev) {
        var confirm = $mdDialog.confirm().title('Xác nhận đăng xuất')
            .content('Bạn có chắc chắn muốn đăng xuất khỏi người dùng : ' + root.getSessionName() + ' ?')
            .targetEvent(ev)
            ...
            $mdToast.show($mdToast.simple().content('Đăng xuất thành công').position('bottom right'));
        });
    };
    ...
    root.showConfirmDialogSimple = function (title, msg, callback) {
        var confirm = $mdDialog.confirm().title(title).content(msg).ok('ĐỒNG Ý');
    ...
    root.showConfirmDialog = function (title, msg, callback) {
        ...
        confirm.ok('ĐỒNG Ý').cancel('HUỶ');
        $mdDialog.show(confirm).then(callback);
    };

    // show dialog info
    root.showInfoDialogSimple = function (title, msg) {
        var alert = $mdDialog.alert().title(title).content(msg).ok('ĐÓNG');
        $mdDialog.show(alert)
    };

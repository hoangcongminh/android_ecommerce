angular.module('App').controller('AddOrderController', function ($rootScope, $scope, $http, $mdToast, $mdDialog, $route, $timeout, request) {
	...
	root.pagetitle 			= (is_new) ? 'Thêm đơn hàng' : 'Sửa đơn hàng';
	self.button_text 		= (is_new) ? 'LƯU' : 'CẬP NHẬT';
	self.is_new 			= angular.copy(is_new);
	self.submit_loading 	= false;
	self.status_array 		= (is_new) ? ["ĐANG CHỜ"] : ["ĐANG CHỜ", "HUỶ"];
	self.shipping_array 	= [];
	...
	if (is_new) {
		original = {
			...
			comment: null, status: "ĐANG CHỜ", total_fees: 0, tax:0, serial: null, created_at: now, last_update: now
		};

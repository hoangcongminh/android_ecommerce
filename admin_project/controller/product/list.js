angular.module('App').controller('ProductController', function ($rootScope, $scope, $http, $mdToast, $mdDialog, $route, request) {
	...
	request.getAllCategory().then(function(resp){
		var temp_category = {id:-1, name:'Tất cả danh mục'};
	...
	self.deleteProduct = function(ev, p) {
		var confirm = $mdDialog.confirm().title('Xác nhận xoá');
			confirm.content('Bạn có chắc muốn xoá sản phẩm : '+p.name+' ?');
		...
		$mdDialog.show(confirm).then(function() {
			request.deleteOneProduct(p.id).then(function(res){
				if(res.status == 'success'){
                    ...
                    root.showConfirmDialogSimple('', 'Xoá sản phẩm '+p.name+' <b>Thành công</b>!', function(){
                        window.location.reload();
                    });
				}else{
                    root.showInfoDialogSimple('', 'Lỗi , <b>Xoá thất bại</b> sản phẩm '+p.name);
				}
			});
		});
    ...
    self.publishDialog = function (ev, o) {
        $mdDialog.show({
            ...
            '   <p>Bạn có chắc muốn phát hành sản phẩm : <b>{{obj.name}}</b> ?</p><br>' +
            '   <md-checkbox ng-model="send_notif">Gửi thông báo cho người dùng</md-checkbox>' +
        	...
        	$scope.publish = function() {
                ...
                request.updateOneProduct($scope.obj.id, $scope.obj).then(function(resp){
                    if(resp.status == 'success'){
                        ...
                        root.showConfirmDialogSimple('', 'Phát hành sản phẩm '+obj.name+' <b>Thành công</b>!', function(){
                            window.location.reload();
                        });
                    }else{
                        var failed_txt = 'Lỗi , <b>Phát hành thất bại</b> sản phẩm '+obj.name;

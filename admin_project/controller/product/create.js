angular.module('App').controller('AddProductController', function ($rootScope, $scope, $http, $mdToast, $mdDialog, $route, $timeout, request) {
	...
	root.pagetitle 			= (is_new) ? 'Thêm sản phẩm' : 'Sửa sản phẩm';
	self.button_text 		= (is_new) ? 'LƯU' : 'CẬP NHẬT';
	...
	self.status_array 		= ["CÓ HÀNG", "HẾT HÀNG", "TẠM NGƯNG"];
	...
	self.onFileSelect = function (files) {
		...
		$mdToast.show($mdToast.simple().content("Đã chọn file").position('bottom right'));
	};
	...
	self.deleteImage = function (ev, img) {
		var confirm = $mdDialog.confirm().title('Xác nhận xoá').content('Bạn có chắc muốn xoá hình ảnh ?').targetEvent(ev).ok('ĐỒNG Ý').cancel('HUỶ');
		$mdDialog.show(confirm).then(function () {
			request.deleteProductImageByName(img.name).then(function (res) {
				if (res.status == 'success') {
					$mdToast.show($mdToast.simple().hideDelay(1000).content('Xoá ảnh thành công!').position('bottom right')).then(function () {
						window.location.reload();
					});
				} else {
					$mdToast.show($mdToast.simple().hideDelay(6000).action('CLOSE').content('Lỗi , Xoá ảnh không thành công').position('bottom right')).then(function (response) {});
				}
			});
		}, function () {});
	};

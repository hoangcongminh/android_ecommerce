angular.module('App').controller('NewsController', function ($rootScope, $scope, $http, $mdToast, $mdDialog, $route, request) {
	...
	root.pagetitle = 'Tin tức';
	self.loading = true;

	root.search_enable = true;
	root.toolbar_menu = { title: 'Thêm tin tức' }
	...
	self.deleteNewsInfo = function(ev, n) {
		var confirm = $mdDialog.confirm().title('Xác nhận xoá');
			confirm.content('Bạn có chắc chắn muốn xoá tin tức : '+n.title+' ?');
		...
		$mdDialog.show(confirm).then(function() {
			request.deleteOneNewsInfo(n.id).then(function(resp){
				if(resp.status == 'success'){
				    ...
				    root.showConfirmDialogSimple('', 'Xoá tin tức '+n.title+' <b>Thành công</b>!', function(){
				        window.location.reload();
				    });
				}else{
				    var failed_txt = '';
                    if(resp.msg != null) failed_txt += '<br>' + resp.msg;
                    root.showInfoDialogSimple('Xoá thất bại', failed_txt);
				}
			});
		});
    ...
    self.publishDialog = function (ev, o) {
        $mdDialog.show({
            ...
            '   <p>Bạn có chắc chắn muốn phát hành tin tức : <b>{{obj.title}}</b> ?</p><br>' +
            ...
            $scope.publish = function() {
                ...
                request.updateOneNewsInfo($scope.obj.id, $scope.obj).then(function(resp){
                    if(resp.status == 'success'){
                        ...
                        root.showConfirmDialogSimple('', 'Phát hành tin tức '+obj.title+' <b>Thành công</b>!', function(){
                            window.location.reload();
                        });
                    }else{
                        var failed_txt = 'Opps , <b>Phát hành thất bại</b> Tin tức '+obj.title;

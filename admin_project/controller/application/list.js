angular.module('App').controller('ApplicationController', function ($rootScope, $scope, $http, $mdToast, $mdDialog, $cookies, request) {
	...
	root.toolbar_menu = { title: 'Thêm phiên bản' }
	root.pagetitle = 'Ứng dụng';
	...
	self.deleteVersion = function(ev, a) {
		var confirm = $mdDialog.confirm().title('Xác nhận xoá');
			confirm.content('Bạn có chắc chắn muốn xoá phiên bản : '+a.version_code+' ?');
			confirm.targetEvent(ev).ok('ĐỒNG Ý').cancel('HUỶ');

		$mdDialog.show(confirm).then(function() {
			request.deleteOneAppVersion(a.id).then(function(resp){
				if(resp.status == 'success'){
					root.showConfirmDialogSimple('', 'Xoá phiên bản '+a.version_code+' <b>thành công</b>!', function(){
...
function VersionControllerDialog($rootScope, $scope, $mdDialog, request, $mdToast, $route, $timeout, version) {
	...
	self.title = (is_new) ? 'Thêm phiên bản' : 'Sửa phiên bản';
	self.buttonText = (is_new) ? 'LƯU' : 'CẬP NHẬT';
